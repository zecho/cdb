...
CREATE FUNCTION "1".strips(_geek_id INTEGER) RETURNS JSON AS
$$
SELECT array_to_json(array_agg(row_to_json(strip_select))) FROM (
	...
	FROM "1".strip s LEFT JOIN "1".strip_geek sg ON s.id = sg.strip_id WHERE sg.geek_id = 1 OR sg.geek_id IS NULL LIMIT 10
) strip_select;
$$ LANGUAGE SQL
SECURITY DEFINER;

GRANT EXECUTE ON FUNCTION "1".strips(INTEGER) TO maestro;
