...
CREATE OR REPLACE FUNCTION cg.upsert_strip(
	_comic_id UUID,
    _title VARCHAR(63),
    _number INTEGER,
    _url VARCHAR(2083),
...
BEGIN
    LOOP
        ...
        BEGIN
            INSERT INTO cg.strip (comic_id, title, number, url, image_url, thumbnail_image_url, bonus_image_url, alt_text) 
...
BEGIN
    LOOP
        ...
        -- we could get a unique-key failure
        BEGIN
            INSERT INTO cg.strip (id, comic_id, title, number, url, image_url, thumbnail_image_url, bonus_image_url, alt_text) 
...
GRANT EXECUTE ON FUNCTION cg.upsert_strip(UUID, VARCHAR, INTEGER, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT) TO lurker;
GRANT EXECUTE ON FUNCTION cg.upsert_strip(UUID, UUID, VARCHAR, INTEGER, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT) TO lurker;
